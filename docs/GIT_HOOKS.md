...
- Pattern: `<prefix>(<scope>): <description>` (scope optional, for example `chore(deps): ...` or `fix: ...`)
- Allowed prefixes: `content`, `feature`, `fix`, `ci`, `chore`
...
The commit hook validates commit subjects; CI validates the **PR title** separately. Update the PR title in GitHub to match the same `<prefix>(<scope>): <description>` convention.
