...
- If the vectors file is absent, reports the prerequisite and exits successfully without writing, deleting, or replacing `related.json`. On a clean checkout that means no related list; if an older generated file is present, it remains in place and may be stale.
...
- Requires `build/semantic-search/vectors.json` and reads `.njk` post frontmatter in `src/site/posts/` to attach each entry's primary topic.
- Averages entry vectors, uses deterministic classical multidimensional scaling to produce two-dimensional coordinates, and retains links at cosine similarity >= 0.6. Each node nominates its three strongest qualifying neighbours; after duplicate pairs are merged, a node can have degree greater than three because other nodes may nominate it.
...
- If vectors are absent, or fewer than three entries can be projected, reports the condition and exits successfully without writing, deleting, or replacing `semanticMap.json`. An older generated file therefore remains in place if one exists.
...
- If `semanticMap.json` is absent, reports the prerequisite and exits successfully without writing or deleting `storyLab.json`. If vectors are absent but the map exists, it still replaces `storyLab.json` with a reduced artifact: trails contain only their available editorial anchors, `semanticIndex` contains zero counts and no passage echoes, and the radial semantic panel falls back to the whole-entry edges from `semanticMap.json`.
