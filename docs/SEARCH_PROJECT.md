...
| Pages indexed | 194 (including social cards) | 89 (content pages only) |
...
- `eleventy.js` — Replaced `eleventy.after` hook to run `pagefind --site build --exclude-selectors "pre, code"`; added `data-pagefind-ignore` to `codeAndDemo` shortcode
...
- `src/site/search.njk` — Rewritten to use Pagefind JS API with error handling, loading state, XSS-safe rendering
- `src/site/404.njk` — Cleaned up legacy attributes, added `data-pagefind-ignore` wrapper
- `README.md` — Updated search description
...
4. 404 and search pages wrapped in `data-pagefind-ignore` — excluded from results
5. `<pre>` and `<code>` elements excluded globally via `--exclude-selectors` (prevents raw HTML from code examples in results)
6. `codeAndDemo` shortcode output wrapped in `data-pagefind-ignore`
7. After Eleventy builds all HTML, the `eleventy.after` hook runs `pagefind --site build --exclude-selectors "pre, code"`
8. Pagefind generates a chunked index in `build/pagefind/`
9. The search page imports `/pagefind/pagefind.js` as an ES module
10. On search, Pagefind loads only the relevant index chunks and returns highlighted excerpts
...
- [x] 89 pages indexed (social cards, 404, search page excluded)
- [ ] Search page shows results with highlighted excerpts (no raw HTML tags)
