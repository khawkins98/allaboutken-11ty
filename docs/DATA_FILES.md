...
Maps an empty calendar year or consecutive year range to optional explanatory copy when the register timeline collapses that gap.
...
Keys normally match the year or inclusive year range produced by the `collapseEmptyYears` filter. For a range, the filter first checks the exact `YYYY-YYYY` key and then falls back to a key for the range's first year. Values are plain-text captions. The `_comment` key in the current file records maintenance guidance and is not used as a caption.
...
- `src/site/_includes/partials/pulse-strip.njk` puts the selected caption in the gap's `title` tooltip and screen-reader-only text. The visible timeline shows the year or year range, not the caption.
- Edit this file when a known publication gap needs purposeful copy. Keep captions short.
- Deleting or omitting a matching key is safe: the tooltip and accessible text fall back to `no entries` while the visible timeline still shows the year or range.
...
If `build/semantic-search/vectors.json` is absent, the generator exits successfully without writing, deleting, or replacing this file. On a clean checkout no related list is rendered; if a previously generated file exists, templates can continue to read that stale data. Run the prerequisite commands in order when freshness matters.
...
The generator needs `build/semantic-search/vectors.json` and at least three eligible `/posts/` or `/work/` entries. Missing or insufficient input is non-fatal and leaves any existing artifact untouched. For edge selection, every node nominates up to three qualifying neighbours, then duplicate undirected pairs are merged; incoming nominations mean a node's final `degree` can exceed three. Change projection or edge rules in `scripts/compute-semantic-map.mjs`, then regenerate.
...
`semanticMap.json` is required. If it is missing, the command exits successfully without writing or deleting this file. `vectors.json` is optional when running this command alone: without it, the artifact is still replaced, but trails contain only their available editorial anchors, `semanticIndex` reports zero counts and no passage echoes, and the radial semantic panel falls back to whole-entry map edges. A normal `yarn build` always supplies both inputs. Change career bands, narrative lenses, thresholds, or chart construction in `scripts/compute-story-lab.mjs`, then regenerate.
