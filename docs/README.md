...
| [`SCRIPTS.md`](SCRIPTS.md) | Source of truth for `package.json` scripts, the two-pass semantic-data build, and lifecycle behavior | Contributors running local/CI commands | You need the right command, execution order, prerequisites, or expected output |
...
| [`CORPUS_STORY_LAB.md`](CORPUS_STORY_LAB.md) | Working method and interpretation notes for the experimental corpus visualizations | Maintainers of semantic corpus analysis | You are changing Story Lab datasets, thresholds, lenses, or visual narratives |
...
| [`DATA_FILES.md`](DATA_FILES.md) | Schemas, consumers, and editing status for maintained and generated Eleventy global data | Contributors editing site-wide data or the semantic-derived pipeline | You are changing global data, tracing a template value, or regenerating derived corpus artifacts |
...
- **Semantic corpus pipeline:** `SEARCH.md` -> `SCRIPTS.md` -> `DATA_FILES.md` -> `CORPUS_STORY_LAB.md`
